...
def kpi_card(
    ...
    """Return reusable HTML KPI card."""

    accent = accent or BRAND["blue"]

    if accent == BRAND["blue"]:
        soft = "rgba(22,134,217,.12)"
...
def render_kpis(cards):
    """Render KPI cards using Streamlit columns."""
...
def build_timestamp(data_df, date_c, time_c):
    """Safely combine date and optional time columns."""
...
def load_data(
    ...
    """Load Excel data from upload or SharePoint."""
    ...
    # Helpful diagnostic if SharePoint returns HTML instead of Excel
    if (
        "text/html" in content_type
        and len(response.content) < 100000
    ):
        raise ValueError(
            "SharePoint returned an HTML page instead of "
            "the Excel workbook. Please verify that the "
            "SharePoint link allows direct downloading."
        )
...
uploaded = st.sidebar.file_uploader(
    ...
    type=["xlsx", "xls"],
    help=(
        "Upload a current logistics workbook "
        "to replace the live SharePoint dataset."
    ),
)
...
try:
    ...
    st.info(
        "If this is running on Streamlit Cloud, verify "
        "that the SharePoint download link is valid and "
...
if not isinstance(df_raw, pd.DataFrame):
    st.error(
        "The uploaded source did not return a valid dataframe."
    )
    st.stop()
...
if df_raw.empty:
    st.warning(
        "The logistics workbook contains no records."
    )
    st.stop()
...
dispatch_date_col = (
    col_ship
    if col_ship and col_ship in df.columns
    else col_date
)
...
with tab_captains:
    ...
    if not col_captain:
        ...
        st.info(
            "Map the **Captain / Rider** field in the sidebar "
            "to activate this view."
        )

    else:

        cap_df = filtered.dropna(
            subset=[col_captain]
        ).copy()
        ...
        if cap_df.empty:
            ...
            if rank_df.empty:
                ...
                st.info(
                    "Not enough captain data to draw "
