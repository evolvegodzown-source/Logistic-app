...
# ============================================================================
DATA_PATH = (
    "https://drugstock-my.sharepoint.com/:x:/g/personal/"
    "it_drugstoc_com/"
    "IQA5yp0kdh82Ra7YcCr-be0vAXufIjkPsYHD4yoBbt6byhs"
    "?e=MFF4su&download=1"
)

COVER_LOGO_URL = (
    "https://drugstock-my.sharepoint.com/:i:/g/personal/"
    "it_drugstoc_com/"
    "IQCURjcRKFhMQ4HunFjm4IxrAfQqHn3s3TDz3jUrWgzgw5g"
    "?e=dNiGIh&download=1"
)
...
st.markdown(
    ...
    <style>
        ...
        :root {{
            --ds-blue: {BRAND["blue"]};
            --ds-blue-dark: {BRAND["blue_dark"]};
            --ds-green: {BRAND["green"]};
            --ds-navy: {BRAND["navy"]};
            --ds-teal: {BRAND["teal"]};
            --ds-amber: {BRAND["amber"]};
            --ds-red: {BRAND["red"]};

            --ds-page: {THEME["page"]};
            --ds-surface: {THEME["surface"]};
            --ds-surface-2: {THEME["surface_2"]};
            --ds-surface-3: {THEME["surface_3"]};
...
def kpi_card(
    ...
    if accent == BRAND["blue"]:
        soft = "rgba(22,134,217,.13)"
    elif accent == BRAND["green"]:
        soft = "rgba(16,185,129,.13)"
    elif accent == BRAND["amber"]:
        soft = "rgba(245,158,11,.14)"
    elif accent == BRAND["red"]:
...
def build_timestamp(
    ...
    if (
        not date_c
        or date_c not in data_df.columns
    ):
        return pd.Series(
            pd.NaT,
            index=data_df.index,
...
@st.cache_data(
    ttl=300,
    show_spinner="Fetching live logistics data...",
)
def load_data(path):
...
try:
    ...
    st.error(
        "Unable to load the logistics workbook."
    )

    st.info(
        "Please verify that the SharePoint download "
        "link is still active and returns the Excel "
        "workbook directly."
    )
...
# ============================================================================
required = {
    "Client / Facility": auto["client"],
    "Created Date": auto["created_date"],
    "Delivery Status": auto["status"],
}
...
if missing:
    ...
    st.error(
        "The dashboard could not automatically identify "
    ...
    st.info(
        "Please ensure the SharePoint workbook contains "
        "the standard DrugStoc logistics column names."
    )
...
# ============================================================================
# ASSIGN AUTO-DETECTED COLUMNS
# ============================================================================
col_client = auto["client"]
col_value = auto["value"]
col_qty = auto["qty"]
col_date = auto["created_date"]
col_create_time = auto["created_time"]
col_region = auto["region"]
col_status = auto["status"]
col_captain = auto["captain"]
col_order_type = auto["order_type"]
col_ship = auto["ship_date"]
col_dispatch_time = auto["dispatch_time"]
col_deliv = auto["deliv_date"]
col_delivery_time = auto["delivery_time"]
...
df["Delivery_DT"] = build_timestamp(
    df,
    col_deliv,
    col_delivery_time,
)
...
dispatch_date_col = (
    col_ship
    if col_ship
    and col_ship in df.columns
    else col_date
)


df["Dispatch_DT"] = build_timestamp(
    df,
    dispatch_date_col,
    col_dispatch_time,
)
...
df["Creation_Delivery_TAT"] = (
    df["Creation_Delivery_TAT"].apply(
        lambda x:
            x
    ...
    <div class="hero">
        ...
        <div class="hero-top">
            ...
            <div>
                ...
                <div class="eyebrow">
                    Healthcare Supply Chain Monitor
                </div>

                <div class="hero-title">
                    DrugStoc Pharma Logistics Dashboard
                </div>

                <div class="hero-subtitle">
                    Executive visibility across order fulfillment,
                    delivery turnaround, shipment volume and
                    field-captain performance.
                </div>
...
with tab_overview:
    ...
    avg_creation_to_deliv_tat = (
        filtered[
            "Creation_Delivery_TAT"
        ].mean()
    )
    ...
    section_header(
        "Operational KPIs",
        "Core metrics for order volume, value, fulfillment and delivery speed.",
    )
    ...
    with col_a:
        ...
        if reg_summary.empty:
            ...
            show_empty_chart(
                "No regional data is available for the selected filters."
            )
    ...
    with col_b:
        ...
        if status_summary.empty:
            ...
            show_empty_chart(
                "No fulfillment-status data is available for the selected filters."
            )
    ...
    if trend.empty:
        ...
        show_empty_chart(
            "No weekly order data is available for the selected filters."
        )
...
with tab_captains:
    ...
    section_header(
        ...
        "Performance ranking based on delivery success and turnaround time.",
    )
    ...
    if not col_captain:
        ...
        st.info(
            "No Captain / Rider column was automatically detected "
        ...
        if cap_df.empty:
            ...
            st.info(
                "No captain performance records are available "
                ...
                column_config={
                    ...
                    "Success Rate (%)":
                        st.column_config.ProgressColumn(
                            "Success Rate (%)",
                            help=(
                                "Percentage of the captain's "
                                "filtered orders marked as delivered."
                            ),
                            min_value=0,
                            max_value=100,
                            format="%.1f%%",
            ...
            if rank_df.empty:
                ...
                st.info(
                    "Not enough captain data "
                    "to draw the performance chart."
                )
                ...
                >
...

                    {
                        kpi_card(
                            ...
                            fastest_text,
                            (
                                "Lowest average dispatch-to-delivery "
                                "turnaround among captains with valid TAT."
                            ),
                            "⚡",
                            BRAND["blue"],
                        )
...
with tab_data:
    ...
    st.download_button(
        ...
        data=csv,
        file_name=(
            "drugstoc_filtered_logistics_audit.csv"
        ),
        mime="text/csv",
    )
...
st.markdown(
    ...
    <div
        ...
        "
    >
        DrugStoc • Pharma Logistics Intelligence
